...
* **Demo-mode governance & hidden maintenance switch (2026-07-16)** — **AI usage cap in demo**: when `APP_MODE=demo` (`config('app.demo')`) every account is capped at `User::DEMO_AI_REQUEST_LIMIT` AI requests across ALL AI surfaces (student chat/agent, faculty AI assistant, AI generators), enforced by `EnforceDemoAiLimit` middleware (alias `demo.ai.limit`) on request-consuming AI POST endpoints only (send/generate) — never page loads/history reads. Charges up-front (counted regardless of provider success); exhausted → HTTP 429 with `demo_limit_reached` payload. Outside demo mode nothing is metered. Backed by `users.ai_request_count`; `User::hasAiRequestQuota()`/`hasReachedAiRequestLimit()`/`recordAiRequest()`. **Hidden maintenance switch**: `HandleMaintenanceMode` global web middleware, URL-driven — `?live=false` drops the whole app into Maintenance Mode (`resources/js/pages/Maintenance.vue`, HTTP 503), `?live=true` flips back to live; state persisted globally in cache (sticks across all requests/visitors), `?live=true` evaluated first so an operator can always unlock. Default from `config('app.live_default')` (env `APP_LIVE_DEFAULT`, default `true`=live); testing env + `/up` never gated; JSON callers get JSON 503, Inertia/page requests get the Maintenance page. **Demo backfill**: `DemoFeatureShowcaseSeeder` expanded so the three demo accounts showcase every recent feature end-to-end. Tests: `DemoLimitsAndMaintenanceTest`.
* **User activity tracking (2026-07-17)** — Admin → User Activity panel (route `admin.user-activity`, gated `view_all_analytics`, nav under System): who visited, from where (referrer), on what device, IP-derived location. Tracks meaningful page views (public landing + every authenticated page) via `TrackVisit` middleware appended to the web stack, recording in `terminate()` — AFTER the response flushes, so geolocation + DB write add zero page latency; only real navigations (successful GET full-HTML loads + Inertia visits), excluding ajax/JSON/asset/partial-reload/redirect noise + a polling-route blocklist (heartbeat, notifications poll, search, messenger overview). Per visit: user (or guest/null), session id, path, route name, friendly label, referrer (external only — internal dropped), IP, device type (desktop/mobile/tablet/bot), platform, browser, country, city, user agent, timestamp. `app/Services/Tracking/`: `DeviceDetector` (dependency-free UA parser), `GeoLocationService` (free ip-api.com, cached per-IP 30d, private/loopback → "Local", failures → null), `VisitTracker`, `VisitReportService` (filtered feed + stats); model `App\Models\Visit` (append-only, `forRole` scope), migration `create_visits_table`; `Admin/UserActivityController`, page `Admin/UserActivity.vue` (stat cards total/today/unique/guests; device + top-country + top-page breakdowns; filters: user search, role, device, country, date range; paginated table). Retention: `visits:prune` (`config('tracking.retention_days')`, env `VISIT_RETENTION_DAYS` default 90, 0=disabled), scheduled weekly Sun 03:30. Demo data: `VisitSeeder` (~380 realistic visits, guards at 50-row threshold), wired into `DatabaseSeeder`. Tests: none yet (new/uncommitted).

...
const openInChat = (session) => {
    // Archived conversations remain viewable via the chat deep-link.
    window.location.href = `/chat?session=${session.id}`;
};

const restore = async (session) => {
    try {
        await axios.patch(route('chat.session.unarchive', session.id));
...
const remove = async (session) => {
    ...
    try {
        await axios.delete(route('chat.session.destroy', session.id));
        sessions.value = sessions.value.filter((s) => s.id !== session.id);
    ...
    <div>
        ...
        <AppLayout>
            <div class="page-container space-y-6 py-8">
                ...
                >
                    <template #actions>
                        <Link :href="route('chat')" class="ui-btn-secondary">
                ...
                <Card v-if="sessions.length" padding="p-0">
                    <ul class="divide-y divide-line">
                        ...
                        >
                            ...
                            <button class="min-w-0 flex-1 text-left" @click="openInChat(session)" title="View conversation">
                                ...
                                <p class="mt-0.5 flex items-center gap-1.5 text-xs text-content-faint">
                                    <span class="capitalize">{{ (session.mode || 'academic').replace('_', ' ') }}</span>
                                    <span>•</span>
                ...
                <Card v-else padding="p-0">
                    <EmptyState
                        ...
                        :icon="InboxIcon"
                    >
                        <Link :href="route('chat')" class="ui-btn-primary">
                            <ChatBubbleLeftRightIcon class="h-5 w-5" />
                            Go to Chat
                        </Link>
                    </EmptyState>

...
- **Admin** ("Govern & monitor"): user, role & permission matrix · course catalog (incl. prerequisites), sections, terms & student assignment (full sections → FIFO waitlists) · document approval workflow & knowledge base · institution-wide analytics & top queries · AI provider settings, prompts & retrieval tuning · exam-security gate + discussion moderation queue · AI usage monitor with per-user access control · user activity panel (visitors, referrer, device & IP-derived location with breakdowns) · demo-mode AI request cap + hidden maintenance switch · system monitor, departments & announcements.
...
- **Nine features:** User & Role Matrix · Document Approval (→ auto-embed into RAG) · AI Usage Monitor (per-user tokens, block/unblock) · Catalog & Terms (courses + prerequisites, sections, waitlists, rollover) · Announcements (broadcast) · Discussion Moderation (queue of reported posts/comments) · AI Settings (temperature, top-K, threshold, prompts) · User Activity Panel (who/from where/device/IP-location + total/today/unique/guest stats, device & top-country & top-page breakdowns, filterable feed; logged after load, auto-pruned) · Demo Governance (fixed per-account AI-request cap across all AI features + hidden `?live=false/true` maintenance switch flippable from any page).
...
| 19 | Admin Features | 9 admin feature cards |
